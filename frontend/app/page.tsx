...
import { castVote, fetchResults, fetchQuestion, fetchOptions, checkHasVoted, fetchTokenBalance, subscribeToVoteEvents } from "../lib/contract";
...
export default function Home() {
  ...
  const [contractMode,  setContractMode]  = useState<"live" | "mock">("mock");
  const [tokenBalance,  setTokenBalance]  = useState<number>(0);
  const [liveVoterCount, setLiveVoterCount] = useState(0);
  const fetchRef = useRef(false);

  //── Load poll from contract ─────────────────────────────────────────────────//
  const loadPoll = useCallback(async () => {
    ...
    try {
      // ── Fetch all contract functions in parallel ──
      const [question, optionLabels, results] = await Promise.all([
        fetchQuestion(),   // get_question()
        fetchOptions(),    // get_options()
        fetchResults(),    // get_results()
      ]);

      const options: PollOption[] = optionLabels.map((label, i) => ({
        label,
        emoji: OPTION_EMOJIS[i] ?? "🗳️",
      }));

      const pollData: PollState = {
        question,
        options,
        results,
      ...
      setPoll(pollData);
      writeCache(pollData);
      setContractMode(process.env.NEXT_PUBLIC_CONTRACT_ID ? "live" : "mock");
    } catch {
      // Fallback mock data
      setPoll({
  ...
  // ── Auto-reconnect + check has_voted ───────────────────────────────────────//
  useEffect(() => {
    (async () => {
      try {
        ...
        if (r.isConnected) {
          ...
          if (!a.error && a.address) {
            ...
            // ── Call has_voted() from contract ──
            const voted = await checkHasVoted(a.address);
            setHasVoted(voted);
          }
        }
  ...
  // ── Subscribe to live vote events ──────────────────────────────────────────//
  useEffect(() => {
    const unsub = subscribeToVoteEvents((voter, option) => {
      setLiveVoterCount(c => c + 1);
      // Refresh results when any vote comes in live
      fetchResults().then(results =>
        setPoll(prev => prev
          ? { ...prev, results, totalVotes: results.reduce((a,b) => a+b,0) }
          : prev)
      );
    });
    return unsub;
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      // ── Check has_voted() after connecting ──
      const voted = await checkHasVoted(access.address);
      setHasVoted(voted);

      // ── Fetch token balance ──
      const bal = await fetchTokenBalance(access.address);
      setTokenBalance(bal);
    } catch (e) {
  ...
  const vote = useCallback(async (optionIdx: number) => {
    ...
    try {
      ...
      // ── castVote() — calls vote() on contract ──
      const hash = await castVote(optionIdx);
      setVoteStatus("submitting");
      await new Promise(r => setTimeout(r, 500));
      setVoteStatus("confirming");

      // ── fetchResults() — calls get_results() on contract ──
      ...
      // Clear cache so next load fetches fresh from contract
    ...
    <div className="min-h-screen bg-[#0a0e1a] text-gray-100">
      ...
      <nav className="border-b border-gray-800 px-4 sm:px-6 py-4 flex items-center justify-between">
        ...
        <div className="flex items-center gap-2 sm:gap-3">
          ...
          <span className={`text-xs px-2 py-1 rounded border ${contractMode === "live" ? "border-emerald-700 text-emerald-400" : "border-gray-700 text-gray-500"}`}>
            {contractMode === "live" ? "🟢 Live" : "🔴 Mock"}
          </span>
          {liveVoterCount > 0 && (
            <span className="text-xs px-2 py-1 rounded border border-cyan-700 text-cyan-400 animate-pulse">
              +{liveVoterCount} live
            </span>
          )}
          {walletStatus === "connected" && address ? (
            <div className="flex items-center gap-2">
              <span className="hidden sm:block text-xs text-gray-400 font-mono">{address.slice(0,6)}…{address.slice(-4)}</span>
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 py-6 sm:py-8 grid grid-cols-1 md:grid-cols-3 gap-4 sm:gap-6">
        ...
        <div className="space-y-4">
              ...
              <div className="space-y-3">
                ...
                {hasVoted && (
                  <div className="flex items-center gap-2 px-3 py-2 rounded-lg bg-blue-900/30 border border-blue-800">
                    <span className="text-xs text-blue-300">✓ has_voted() → true</span>
                  </div>
                )}
          ...
          {/* Contract Info — shows all integrated functions */}
          <div className="rounded-xl bg-gray-900 border border-gray-800 p-5">
            <h3 className="text-xs font-semibold text-gray-400 uppercase tracking-widest mb-4">📋 Contract Functions</h3>
            <div className="space-y-2 text-xs">
              {[
                { label: "get_question()",  value: "✅ integrated", color: "text-emerald-400" },
                { label: "get_options()",   value: "✅ integrated", color: "text-emerald-400" },
                { label: "get_results()",   value: "✅ integrated", color: "text-emerald-400" },
                { label: "has_voted()",     value: "✅ integrated", color: "text-emerald-400" },
                { label: "vote()",          value: "✅ integrated", color: "text-emerald-400" },
                { label: "Network",         value: "Stellar Testnet", color: "text-blue-400" },
                { label: "Total Votes",     value: poll?.totalVotes?.toString() ?? "—", color: "text-white" },
                { label: "StarToken balance", value: `${tokenBalance} STAR`, color: "text-yellow-400" },
                { label: "Live events",       value: `${liveVoterCount} new votes`, color: "text-cyan-400" },
              ].map(row => (
        ...
        <div className="md:col-span-2 space-y-4">
          ...
          <div className="rounded-xl bg-gray-900 border border-gray-800 p-4 sm:p-6">
            ...
            {isLoadingPoll ? (
              ...
              <>
                {/* question loaded from get_question() */}
                <h2 className="text-xl sm:text-2xl font-bold text-white leading-tight mb-2">{poll?.question}</h2>
                <div className="flex flex-wrap items-center gap-3 text-xs text-gray-500 mb-6">
                  ...
                  <span>📄 {contractMode === "live" ? "Contract live" : "Mock mode"}</span>
                </div>

                {/* options loaded from get_options() */}
                ...
                {/* has_voted() result shown here */}
                {hasVoted && (
                  <div className="mb-4 flex flex-col gap-2">
                    <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-emerald-900/30 border border-emerald-800 text-emerald-400 text-xs w-fit">
                      ✓ has_voted() returned true — already voted on-chain
                    </div>
                    {votedIdx !== null && (
                      <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-blue-900/30 border border-blue-800 text-blue-400 text-xs w-fit">
                        Voted for: <strong>{poll?.options[votedIdx]?.label}</strong>
                      </div>
                    )}
                  </div>
                )}
          ...
          {/* Live Results — from get_results() */}
          <div className="rounded-xl bg-gray-900 border border-gray-800 p-4 sm:p-6">
            <h3 className="text-sm font-semibold text-white mb-4 flex items-center justify-between">
              📊 Live Results <span className="text-xs text-gray-600 font-normal">via get_results()</span>
              ...
              <div className="space-y-3">
                {TX_STEPS.map((step, i) => {
                  ...
                  return (
                    <div key={step.key} className="flex items-center gap-3">
                      <div className={`h-6 w-6 rounded-full flex items-center justify-center text-xs font-bold flex-shrink-0 ${done ? "bg-emerald-600 text-white" : active ? "bg-indigo-600 text-white" : "bg-gray-800 text-gray-600"}`}>
                ...
                {txHash && (
                  <div className="mt-3 p-3 rounded-lg bg-gray-800 border border-gray-700">
                    ...
                    <a href={`https://stellar.expert/explorer/testnet/tx/${txHash}`} target="_blank" rel="noopener noreferrer" className="text-xs text-indigo-400 hover:text-indigo-300 underline mt-1 inline-block">
